...
export class MirrorSyncProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    if (!mirrorUrl || !mirrorToken) {
      ...
      this.logger.debug(
        'Sincronización abortada silente: Las variables MIRROR_VPS_URL o MIRROR_SYNC_TOKEN no han sido configuradas en el .env',
      );
      return;
    }

    this.logger.log(
      `Procesando envío de sincronización espejo: ${model} [${action}] (Intento actual: ${job.attemptsMade})`,
    );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Mirror VPS rechazó el payload con status Code HTTP ${response.status} - Motivo: ${errText}`,
        );
      }
