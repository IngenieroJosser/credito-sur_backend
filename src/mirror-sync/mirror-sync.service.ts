...
export class MirrorSyncService {
  ...
  async handleDatabaseWriteEvent(payload: {
    ...
    try {
      this.logger.debug(
        `Encolando sync para ${payload.model} [${payload.action}]`,
      );

      await this.syncQueue.add('sync-record', payload, {
      ...
      this.logger.error(
        'Error crudo al insertar trabajo de sincronización en la cola BullMQ',
        err,
      );
    }
  }
