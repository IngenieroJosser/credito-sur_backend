...
export class BackupExcelService {
  ...
  async exportSnapshotToXlsx(filePath: string): Promise<ExportResult> {
    ...
    // ══ 1. CLIENTES ══════════════════════════════════════════════════════════
    {
      ...
      data.forEach((c: any, i: number) => {
        const row = ws.addRow([
          ...
          c.direccion || '',
          c.referencia1Nombre || '',
          c.referencia1Telefono || '',
          c.referencia2Nombre || '',
          c.referencia2Telefono || '',
          fmtE(c.nivelRiesgo),
          fmtE(c.estadoAprobacion),
          c.enListaNegra ? 'SÍ' : 'NO',
          c.puntaje,
          c.categoria?.nombre || '',
          fmtDT(c.creadoEn),
        ]);
        ...
        if (c.enListaNegra)
          row.getCell(14).font = { bold: true, color: { argb: 'FFDC2626' } };
      });
    ...
    // ══ 2. CRÉDITOS ══════════════════════════════════════════════════════════
    {
      ...
      const ws = mkSheet(
        ...
        [
          ...
          { key: 'plazo', width: 10 },
          { key: 'cuotas', width: 10 },
          { key: 'interesTotal', width: 15 },
          { key: 'saldo', width: 15 },
          { key: 'pagado', width: 15 },
          { key: 'inicio', width: 14 },
          { key: 'primerCobro', width: 14 },
          { key: 'fin', width: 14 },
          { key: 'producto', width: 24 },
          { key: 'creadoPor', width: 24 },
      ...
      data.forEach((p: any, i: number) => {
        ...
        const row = ws.addRow([
          ...
          fmtE(p.tipoPrestamo),
          fmtE(p.estado),
          fmtE(p.estadoAprobacion),
          fmtE(p.frecuenciaPago),
          m,
          n2(p.tasaInteres),
          p.plazoMeses,
          p.cantidadCuotas,
    ...
    // ══ 3. CUOTAS ════════════════════════════════════════════════════════════
    {
      ...
      data.forEach((q: any, i: number) => {
        ...
        const row = ws.addRow([
          ...
          n2(q.montoCapital),
          n2(q.montoInteres),
          n2(q.montoInteresMora),
          n2(q.montoPagado),
          fmtD(q.fechaVencimiento),
          q.fechaPago ? fmtDT(q.fechaPago) : '',
        ]);
        rowStyle(row, i);
        numCol(row, 5, 6, 7, 8, 9);
        const bg = EF[String(q.estado)];
        if (bg)
          row.getCell(4).fill = {
            type: 'pattern',
            pattern: 'solid',
    ...
    // ══ 5. DETALLE DE PAGOS ══════════════════════════════════════════════════
    {
      ...
      addTotals(
        ws,
        `TOTALES — ${data.length} detalles`,
        [null, null, sK, sI, sM, sK + sI + sM],
        [4, 5, 6, 7],
      );
    ...
    // ══ 9. TRANSACCIONES ═════════════════════════════════════════════════════
    {
      ...
      data.forEach((t: any, i: number) => {
        ...
        const row = ws.addRow([
          ...
          n2(t.monto),
          t.descripcion || '',
          t.tipoReferencia ? fmtE(t.tipoReferencia) : '',
          nom(t.creadoPor),
          nom(t.aprobadoPor),
          fmtDT(t.fechaTransaccion),
          fmtDT(t.creadoEn),
        ]);
        rowStyle(row, i);
        numCol(row, 4);
        const bg = TF[String(t.tipo)];
        if (bg)
          row.getCell(3).fill = {
            type: 'pattern',
            pattern: 'solid',
    ...
    // ══ 10. GASTOS ═══════════════════════════════════════════════════════════
    {
      ...
      data.forEach((g: any, i: number) => {
        ...
        const row = ws.addRow([
          ...
          n2(g.monto),
          g.descripcion || '',
          fmtE(g.estadoAprobacion),
          nom(g.aprobadoPor),
          fmtDT(g.fechaGasto),
        ]);
        rowStyle(row, i);
        numCol(row, 6);
        const bg = EF[String(g.estadoAprobacion)];
        if (bg)
          row.getCell(8).fill = {
            type: 'pattern',
            pattern: 'solid',
    ...
    // ══ 11. INVENTARIO ═══════════════════════════════════════════════════════
    {
      ...
      data.forEach((p: any, i: number) => {
        ...
        if (p.stock <= p.stockMinimo)
          row.getCell(7).font = { bold: true, color: { argb: 'FFD97706' } };
      });
    ...
    // ══ 14. EXTENSIONES DE PAGO ══════════════════════════════════════════════
    {
      ...
      const ws = mkSheet(
        ...
        [
          ...
          { key: 'cliente', width: 26 },
          { key: 'numCuota', width: 12 },
          { key: 'vencOriginal', width: 14 },
          { key: 'nuevaFechaVenc', width: 14 },
          { key: 'razon', width: 36 },
          { key: 'aprobadoPor', width: 26 },
          { key: 'creadoEn', width: 22 },
        ] as any,
      ...
      data.forEach((e: any, i: number) => {
        const row = ws.addRow([
          ...
          e.cuota?.numeroCuota || '',
          fmtD(e.fechaVencimientoOriginal),
          fmtD(e.nuevaFechaVencimiento),
          e.razon || '',
          nom(e.aprobadoPor),
          fmtDT(e.creadoEn),
        ]);
    ...
    // ══ 15. USUARIOS DEL SISTEMA ═════════════════════════════════════════════
    {
      ...
      data.forEach((u: any, i: number) => {
        const row = ws.addRow([
          ...
          fmtE(u.estado),
          u.esPrincipal ? 'SÍ' : 'NO',
          u.ultimoIngreso ? fmtDT(u.ultimoIngreso) : 'Nunca',
          u.creadoPorId || '',
          fmtDT(u.creadoEn),
        ]);
        rowStyle(row, i);
        const bg = EF[String(u.estado)];
        if (bg)
          row.getCell(6).fill = {
            type: 'pattern',
            pattern: 'solid',
        ...
        if (u.esPrincipal) {
          row.getCell(7).font = { bold: true, color: { argb: 'FF004F7B' } };
          row.getCell(7).fill = {
            type: 'pattern',
            pattern: 'solid',
            fgColor: { argb: 'FFD6E9F5' },
          };
        }
      });
