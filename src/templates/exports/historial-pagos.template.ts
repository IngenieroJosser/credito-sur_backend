...
export async function generarPDFPagos(
  ...
  const drawPageHeader = (): number => {
    ...
    doc
      .fontSize(22)
      .font('Helvetica-Bold')
      .fillColor('#1A5F8A') // Volvemos a Bold pero mantenemos mayúsculas/minúsculas
    ...
    [
      {
        ...
        val: totales.totalRecaudado,
        color: AZUL_DARK,
        bg: '#D6E9F5',
      },
      {
        label: 'TOTAL CAPITAL',
        val: totales.totalCapital ?? 0,
        color: GRIS_TXT,
        bg: '#F0F4F8',
      },
      {
        label: 'TOTAL INTERESES',
        val: totales.totalIntereses ?? 0,
        color: NAR_DARK,
        bg: '#FDE8D5',
      },
      {
        label: 'TOTAL MORA',
        val: totales.totalMora ?? 0,
        color: GRIS_TXT,
        bg: '#F0F4F8',
      },
    ].forEach((m, i) => {
  ...
  filas.forEach((fila, i) => {
    ...
    rowVals.forEach((val, ci) => {
      ...
      if (ci === 5) {
        ...
        doc
          .font('Helvetica-Bold')
          .fillColor(fila.esAbono ? NAR_DARK : AZUL_DARK);
      } else if (ci === 4) {
        // BADGE Tipo de pago
        doc
          .font('Helvetica-Bold')
