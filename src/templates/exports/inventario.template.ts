...
export async function generarExcelInventario(
  ...
  tRow.eachCell({ includeEmpty: true }, (cell, cn) => {
    ...
    cell.font = {
      bold: true,
      color: { argb: cn <= 5 ? XL.BLANCO : XL.GRIS_TEXTO },
    };
    cell.border = {
...
export async function generarPDFInventario(
  ...
  const drawHeader = (): number => {
    doc
      .fontSize(22)
      .font('Helvetica-Bold')
      .fillColor(PDF.AZUL_DARK)
      .text('Créditos del Sur', TABLE_LEFT, 25);
    doc
      .fontSize(9)
      .font('Helvetica')
      .fillColor(PDF.NAR_MED)
      .text('REPORTE DE INVENTARIO', TABLE_LEFT, 52, { characterSpacing: 0.5 });

    // Badge fecha
    doc
      .roundedRect(PW - 180, 20, 148, 44, 5)
      .fillAndStroke(PDF.BLANCO, PDF.GRIS_LINEA);
    doc
      .fontSize(8)
      .font('Helvetica-Bold')
      .fillColor(PDF.GRIS_MED)
    ...
    // Banda de contexto
    doc
      .roundedRect(TABLE_LEFT, 76, PW - 60, 36, 8)
      .fillAndStroke(PDF.AZUL_PALE, PDF.GRIS_LINEA);
    doc
      .fontSize(9)
      .font('Helvetica')
      .fillColor(PDF.GRIS_TXT)
      .text(
    ...
    const cards = [
      {
        ...
        val: String(totales.totalProductos),
        bg: PDF.AZUL_SOFT,
        color: PDF.AZUL_DARK,
      },
      {
        label: 'BAJO STOCK',
        val: String(totales.productosBajoStock),
        bg: PDF.ROJO_PALE,
        color: PDF.ROJO_DARK,
      },
      {
        label: 'VALOR INVENTARIO',
        val: fmtCOP(totales.totalValorInventario),
        bg: PDF.NAR_SOFT,
        color: PDF.NAR_DARK,
      },
    ];
  ...
  filas.forEach((fila, i) => {
    ...
    const hNombre = doc.heightOfString(vals[1], {
      width: PDF_COLS[1].w - 6,
      lineBreak: true,
    });
    ...
    vals.forEach((v, ci) => {
      ...
      } else if ((ci === 4 || ci === 5) && lowStock) {
        // Stock/Mín bajo stock
        doc.font('Helvetica-Bold').fillColor(PDF.ROJO_DARK);
      } else if (ci === 6 && !fila.activo) {
        // Inactivo: gris
  ...
  doc
    ...
    .fillColor(PDF.NAR_MED)
    .fontSize(8)
    .text(fmtCOP(totales.totalValorInventario), costoX + 2, y + 9, {
      width: PDF_COLS[3].w - 4,
      align: 'right',
    });
