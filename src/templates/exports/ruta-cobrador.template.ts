...
export async function generarPDFRutaCobrador(
  ...
  const drawHeader = (): number => {
    ...
    doc
      .roundedRect(W - 180, 20, 148, 44, 5)
      .fillAndStroke(C.BLANCO, C.GRIS_LINEA);
    doc
      .fontSize(8)
      .font('Helvetica-Bold')
      .fillColor(C.GRIS_MED)
    ...
    doc
      .roundedRect(30, 76, W - 60, 44, 8)
      .fillAndStroke(C.AZUL_PALE, C.GRIS_LINEA);
    doc
      .fontSize(10)
      .font('Helvetica-Bold')
      .fillColor(C.AZUL_DARK)
  ...
  filas.forEach((fila, i) => {
    ...
    vals.forEach((v, ci) => {
      ...
      if (ci === 8) {
        ...
        const esAusente =
          String(fila.estadoVisita || '').toLowerCase() === 'ausente';
        doc
          .font('Helvetica-Bold')
          .fillColor(esAusente ? '#E11D48' : C.GRIS_MED);
      } else {
        doc
      ...
      const yText = canWrap ? y + 4 : y + Math.max(4, (rowH - textH) / 2);
