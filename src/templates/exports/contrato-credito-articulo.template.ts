...
const fmtCOP = (v: number): string => `$${(v || 0).toLocaleString('es-CO')}`;
...
export async function generarContratoPDF(
  ...
  const fieldLine = (label: string, value: string, indent = ML): number => {
    doc
      .font('Helvetica-Bold')
      .fontSize(10)
      .fillColor(C.GRIS_TXT)
      .text(label, indent, y, { continued: true });
    doc.font('Helvetica').fillColor(C.NEGRO).text(`  ${value}`);
    y = doc.y + 4;
    return y;
  };
  ...
  const blankLine = (label: string, indent = ML, lineLen = LINE): number => {
    doc
      ...
      .fillColor(C.GRIS_TXT)
      .text(label, indent, y, { continued: true });
    doc.font('Helvetica').fillColor(C.NEGRO).text(`  ${lineLen}`);
    y = doc.y + 5;
    return y;
  };
  ...
  if (esCredito) {
    doc
      ...
      .fillColor(C.NEGRO)
      .text(
        'El ACREEDOR otorga a EL CLIENTE un crédito para la adquisición del siguiente artículo:',
        ML,
        y,
        { width: TW },
      );
  ...
  const bullets: Array<[string, string]> = [
    ...
    [
      'Referencia personal 1 (nombre y teléfono):',
      data.referencia1 || '_'.repeat(26),
    ],
    [
      'Referencia personal 2 (nombre y teléfono):',
      data.referencia2 || '_'.repeat(26),
    ],
  ];
  ...
  bullets.forEach(([label, val]) => {
    ...
    doc.font('Helvetica').fillColor(C.NEGRO).text(`  ${val}`);
    y = doc.y + 2;
  });
  y += 4;
  ...
  doc.font('Helvetica').fillColor(C.NEGRO).text(`  ${lugarFechaText}`);
  y = doc.y + 24;
  ...
  doc
    ...
    .fontSize(10.5)
    .fillColor(C.NEGRO)
    .text('FIRMA DEL ACREEDOR (CRÉDITOS DEL SUR)', COL2, yFirmas, {
      width: CW,
    });
  doc.font('Helvetica').fontSize(10.5).fillColor(C.NEGRO);
  let yF2 = doc.y + 4;
