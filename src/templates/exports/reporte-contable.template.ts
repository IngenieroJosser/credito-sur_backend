...
export async function generarPDFContable(
  ...
  const drawPageHeader = (): number => {
    ...
    [
      ...
      {
        label: 'TRANSACCIONES',
        val: String(transacciones.length),
        bg: '#F0F4F8',
        color: GRIS_TXT,
        isNum: false,
      },
  ...
  cajas.forEach((caja, i) => {
    ...
    vals.forEach((v, ci) => {
      const align =
        ci >= 5 ? 'right' : ci === 1 || ci === 2 ? 'center' : 'left';
  ...
  transacciones.forEach((t, i) => {
    ...
    vals.forEach((v, ci) => {
      ...
      if (ci === 1) {
        doc
          .font('Helvetica-Bold')
          .fillColor(t.tipo === 'INGRESO' ? '#059669' : '#DC2626');
      } else if (ci === 2) {
        doc.font('Helvetica-Bold').fillColor(AZUL_DARK);
      } else if (ci === 3) {
        doc.font('Helvetica-Bold').fillColor(GRIS_TXT);
  ...
  doc
    ...
    .text(
      ...
      tableLeft,
      y,
      { align: 'center', width: movTableWidth },
    );
