...
export async function generarExcelClientes(
  ...
  [11, 12].forEach((c) => {
    // These indices correspond to the formula columns (K and L)
...
export async function generarPDFClientes(
  ...
  const drawPageHeader = (): number => {
    ...
    [
      ...
      },
      {
        ...
        val: String(filas.filter((f) => f.prestamosActivos > 0).length),
        bg: '#F0F4F8',
        color: GRIS_TXT,
        isNum: false,
      },
  ...
  filas.forEach((fila, i) => {
    ...
    vals.forEach((v, ci) => {
      const align =
        ci >= 7 ? 'right' : ci === 4 || ci === 6 ? 'center' : 'left';
