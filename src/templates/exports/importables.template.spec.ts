...
describe('Plantillas importables de exportacion', () => {
  ...
  it('no exporta un articulo con mas plazos de los que caben en la hoja', async () => {
    // La hoja admite 3 opciones de plazo por decision del negocio, y en la base
    // hay un articulo con 4 (1, 2, 6 y 12 meses). Antes se exportaba recortado
    // y el plazo sobrante se perdia sin avisar; ahora no se genera el archivo.
    await expect(
      generarExcelInventarioImportable(
        [articuloBase],
        [
    ...
    ).rejects.toThrow(/CEL-A15.*4 plazos.*admite 3/s);
  });
  ...
  it('conserva el precio de contado y varias opciones de plazo en el viaje de ida y vuelta', async () => {
    ...
    expect(resultado.precios?.map((p) => p.meses).sort()).toEqual([0, 1, 2, 3]);
    // La utilidad se calcula sobre el costo para poder revisarla antes de importar.
    expect(resultado.precios?.find((p) => p.meses === 0)?.utilidad).toBe(60000);
  });
