...
export async function generarExcelCartera(
  ...
  ws.mergeCells('H3:T3');
  ws.getCell('H3').value =
    `Total registros: ${totales.totalRegistros}  |  Fecha: ${fecha}`;
  ...
  const kpis = [
    {
      ...
      val: totales.montoTotal,
      bg: 'FFD6E9F5',
      color: AZUL,
    },
    {
      label: 'CAPITAL ACTUAL',
      val: totales.montoPendiente,
      bg: 'FFF0F4F8',
      color: GRIS_OSC,
    },
    {
      label: 'CAPITAL PAGADO',
      val: totales.montoPagado,
      bg: 'FFF0F4F8',
      color: GRIS_OSC,
    },
    {
      label: 'INTERÉS RECOGIDO',
      val: totales.interesRecogido,
      bg: NARANJA_CLARO,
      color: NARANJA,
    },
    {
  ...
  Object.entries(porEstado).forEach(([estado, d], i) => {
    ...
    row.height = 18;
    const bg =
      estadoFill[estado.toUpperCase()] ||
      (i % 2 === 0 ? 'FFF8FAFC' : 'FFFFFFFF');
    row.eachCell((cell) => {
...
export async function generarPDFCartera(
  ...
  filas.forEach((fila, i) => {
    ...
    vals.forEach((v, ci) => {
      const align =
        ci >= 3 && ci <= 8 ? 'right' : ci === 9 || ci === 2 ? 'center' : 'left';
  ...
  [
    ...
    if (ci < cols.length) {
      doc
        .fillColor(val > 0 && ci === 8 ? NAR_SOFT : BLANCO)
        .font('Helvetica-Bold')
        .fontSize(8);
      doc.text(fmtCOP(val), tx + 4, y + 9, {
        width: cols[ci].width - 8,
        align: 'right',
      });
      tx += cols[ci].width;
