  ...
  const kpis = [
    {
      ...
      val: totales.totalVencido,
      bg: 'FFF5F3FF',
      color: 'FF7C3AED',
    },
    {
      label: 'Monto Original',
      val: totales.totalMontoOriginal,
      bg: 'FFF8FAFC',
      color: 'FF475569',
    },
    {
      label: 'Intereses de Mora',
      val: totales.totalInteresesMora,
      bg: 'FFF8FAFC',
      color: 'FF475569',
    },
    {
      label: 'Promedio Días Venc.',
      val: totales.diasPromedioVencimiento,
      bg: 'FFF5F3FF',
      color: 'FF7C3AED',
    },
  ];
...
export async function generarPDFVencidas(
  ...
  const drawPageHeader = (): number => {
    ...
    [
      {
        label: 'SALDO VENCIDO TOTAL',
        val: fmtCOP(totales.totalVencido),
        bg: PURPLE_PALE,
        color: PURPLE_DARK,
        isNum: true,
      },
      {
        label: 'MONTO ORIGINAL',
        val: fmtCOP(totales.totalMontoOriginal),
        bg: '#F0F4F8',
        color: GRIS_TXT,
        isNum: true,
      },
      {
        ...
        val: fmtCOP(totales.totalInteresesMora),
        bg: '#FEF2F2',
        color: ROJO_DARK,
        isNum: true,
      },
      {
        label: 'PROMEDIO DÍAS VENC.',
        val: String(totales.diasPromedioVencimiento),
        bg: '#F0F4F8',
        color: GRIS_TXT,
        isNum: false,
      },
  ...
  filas.forEach((fila, i) => {
    ...
    vals.forEach((v, ci) => {
      const align =
        ci >= 4 && ci <= 6 ? 'right' : ci === 3 || ci === 7 ? 'center' : 'left';
