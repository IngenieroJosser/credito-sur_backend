...
describe('LoansService accounting impact for approved loans', () => {
  ...
  it('no falla la creación pendiente si una notificación posterior a la aprobación falla', async () => {
    mockConfig.shouldAutoApproveCredits.mockResolvedValueOnce(false);
    mockPush.sendPushNotification.mockRejectedValueOnce(
      new Error('push temporalmente no disponible'),
    );
