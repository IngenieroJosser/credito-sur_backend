...
export class InventoryService {
  ...
  async create(createInventoryDto: CreateInventoryDto) {
    try {
      ...
      const preciosData = createInventoryDto.precios ? [...createInventoryDto.precios] : [];
      ...
      if (createInventoryDto.precioContado !== undefined) {
          ...
          if (!hasContado) {
              preciosData.push({ meses: 0, precio: createInventoryDto.precioContado });
          }
      }
      ...
      if (categoriaId) {
        const cat = await (this.prisma as any).categoria.findUnique({ where: { id: categoriaId } });
        if (cat) {
         ...
         const cat = await (this.prisma as any).categoria.findFirst({ 
             where: { nombre: { equals: createInventoryDto.categoria, mode: 'insensitive' } } 
         });
         if (cat) {
             categoriaId = cat.id;
  ...
  async update(id: string, updateInventoryDto: UpdateInventoryDto) {
    ...
    if (updateInventoryDto.codigo && updateInventoryDto.codigo !== existingProduct.codigo) {
       const duplicate = await this.prisma.producto.findUnique({
         where: { codigo: updateInventoryDto.codigo }
       });
       if (duplicate) throw new ConflictException('El código de producto ya existe');
    }
    ...
    try {
      ...
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (catId !== undefined || catName !== undefined) {
             ...
             if (catId) {
                 const cat = await (tx as any).categoria.findUnique({ where: { id: catId } });
                 if (cat) {
                     catName = cat.nombre;
                 } else {
        ...
        if (updateInventoryDto.precios || updateInventoryDto.precioContado !== undefined) {
           ...
           if (updateInventoryDto.precioContado !== undefined) {
               // Update or create precioContado (meses=0)
                await tx.precioProducto.upsert({
                    ...
                    create: { productoId: id, meses: 0, precio: updateInventoryDto.precioContado }
                });
           }
        }
